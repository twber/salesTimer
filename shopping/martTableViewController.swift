...
class martTableViewController: UITableViewController {
    ...
    func timemove(){
        ...
        timeMove = Timer.scheduledTimer(withTimeInterval: 1, repeats: true, block: { (_) in
            ...
            let yearDateComponents = DateComponents(calendar: Calendar.current, year: 2020, month: 09, day: 01, hour: 00, minute: 00, second: 00)
            let yearDate = yearDateComponents.date
            let interval = yearDate!.timeIntervalSince(nowTime)
            self.dayLabel.text = String(Int(interval/86400))
            self.hourLabel.text = String(Int(interval/3600)%24)
            self.minLabel.text = String(Int(interval/60)%60)
